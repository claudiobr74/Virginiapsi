...
begin
  ...
  select cs.id, cs.organization_id, cs.patient_id, cs.version
    into sess
  ...
  select a.id, a.organization_id, a.run_id, a.type, a.structured_content, a.review_status
    into artifact_row
  from public.ai_artifacts a
  where a.id = p_artifact_id;

  if artifact_row.id is null then
    raise exception 'ai artifact not found' using errcode = 'P0002';
  end if;

  select r.id, r.organization_id, r.patient_id, r.session_id, r.purpose
    into run_row
  ...
  if artifact_row.review_status <> 'pending' then
    raise exception 'ai artifact already reviewed' using errcode = 'P0001';
  end if;
