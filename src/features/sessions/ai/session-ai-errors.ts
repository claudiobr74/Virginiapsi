...
export function isInvalidGeminiModelError(error: unknown): boolean {
  ...
  return error.providerCode === "NOT_FOUND";
}
...
export function classifyAiRunInsertError(
  error: { code?: string | null; message?: string | null } | null | undefined,
): { dbCode?: string; constraint?: string } {
  if (!error) {
    return {};
  }
  const dbCode =
    typeof error.code === "string" && /^[A-Z0-9_]{1,32}$/i.test(error.code)
      ? error.code
      : undefined;
  const constraintMatch = error.message?.match(/constraint\s+"([a-z0-9_]+)"/i);
  const constraint =
    constraintMatch && /^[a-z0-9_]{1,64}$/.test(constraintMatch[1])
      ? constraintMatch[1]
      : undefined;
  return { dbCode, constraint };
}
