...
describe("foldFinalizedClinicalSessionLast", () => {
  ...
  it("ignora in_progress e sessão finalizada no futuro", () => {
    const lastByPatient = foldFinalizedClinicalSessionLast(
      [
        {
          patient_id: PATIENT,
  ...
  it("usa started_at como fallback quando ended_at não existe", () => {
    const lastByPatient = foldFinalizedClinicalSessionLast(
      [
        {
          patient_id: PATIENT,
...
describe("listPatientDirectory query contract", () => {
  it("usa sessão clínica finalizada para Última e appointment válido para Próxima", () => {
