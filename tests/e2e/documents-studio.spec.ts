...
async function confirmStudioIssue(page: Page, options?: { clinical?: boolean; certificate?: boolean; laudo?: boolean }) {
  await page.getByLabel("Preview conferido").check();
...
test.describe("Estúdio de documentos", () => {
  test("declaração: gerar, preview, emitir PDF e registrar entrega", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Estúdio de Documentos" })).toBeVisible();
    await page.getByRole("link", { name: "Novo documento" }).click();
    await page.getByLabel("Modelo").selectOption("declaration_attendance");
    ...
    await page.getByRole("button", { name: "Gerar estrutura" }).click();
  ...
  test("relatório psicológico: editar, preview, revisar e emitir (IA opcional)", async ({ page }) => {
    await loginViaUi(page);
    await page.goto("/app/documents");
    await page.getByPlaceholder(/Buscar documento/).fill("relatório para psiquiatra");
    await expect(page.getByRole("heading", { name: "Relatório para psiquiatra" })).toBeVisible();
  ...
  test("contrato psicoterapêutico completo em livreto: preview multipágina e emissão", async ({ page }) => {
    ...
    await page.getByLabel("Formato").selectOption("livreto");
